"""dataset_template collage_stages: full-body outfit + activity diversity

Reason: the 0023 recipe's stage 4 ("Full-body Lifestyle") only produced ~6 of
~54 total training tiles as full-body shots, and those 6 tiles shared a narrow
"everyday casual" wardrobe with no deliberate outfit or activity variety. A
LoRA trained on this recipe under-represents full-body poses, outfit styles,
and real-world activities. This migration keeps stages 1-3 (face rotation,
expressions, angles/upper-body/wardrobe) exactly as-is — they are already
balanced — and replaces the single full-body stage with TWO dedicated
full-body stages:

  * Stage 4 "Full-body Outfit Styles (portrait)" — same 3x2 portrait geometry
    as the old stage 4, but each of the 6 tiles is assigned a DIFFERENT named
    outfit style (classy formal, business casual, comfy casual, streetwear,
    sporty athleisure, summer casual) with its own setting, instead of a
    generic "everyday outfit" wardrobe.
  * Stage 5 "Full-body Activities (portrait)" — same geometry again, each
    tile assigned a DIFFERENT everyday activity (walking, jogging/stretching,
    yoga, cafe seating, shopping/browsing, cooking/plants) paired with a
    matching outfit, mandating natural motion, plausible hands, and balanced
    left/right orientation.

Geometry (size/grid/inset/reference_policy) for the new stages is unchanged
from 0023's stage 4; only prompt content changes plus one appended stage.
...
Using the PRIMARY references to lock facial identity and the SECONDARY references only as supporting context, generate ONE high-resolution PORTRAIT image containing a clean 3x2 grid (3 columns, 2 rows = 6 tall vertical tiles) of the SAME person, fully clothed, wearing SIX distinct outfit styles across varied real-world settings.
...
OUTFIT STYLE (one per tile — EACH tile a DIFFERENT outfit STYLE from this list — never repeat a style): (1) classy formal — tailored blazer or elegant cocktail dress, upscale interior; (2) business casual — blouse and slacks, office; (3) comfy casual — oversized knit sweater and leggings, home sofa; (4) streetwear — hoodie, jeans, sneakers, urban street; (5) sporty athleisure — leggings and zip jacket, park; (6) summer casual — sundress, cafe terrace. Every outfit fully clothed with a relaxed, natural fit — nothing tight or body-emphasizing. POSE: standing straight-on; walking mid-stride; seated; 3/4 turned; candid (leaning, hands in pockets, adjusting bag/hair).
...
LOCK (identical): facial identity and ALL skin characteristics; hair color/texture/shine/thickness/length; natural realistic body build and proportions. VARY only: outfit style (per the list above), setting/background, full-body pose, body/face orientation (balanced left/right), camera height/distance (within the framing distribution), lighting, hairstyle ARRANGEMENT.
...
Using the PRIMARY references to lock facial identity and the SECONDARY references only as supporting context, generate ONE high-resolution PORTRAIT image containing a clean 3x2 grid (3 columns, 2 rows = 6 tall vertical tiles) of the SAME person, fully clothed, engaged in SIX distinct everyday activities across varied real-world settings.
...
ACTIVITY (one per tile — EACH tile a DIFFERENT activity from this list — never repeat an activity): (1) walking mid-stride at a crosswalk, streetwear; (2) jogging or a post-run stretch outdoors, sportswear; (3) a yoga pose on a mat, athletic wear; (4) seated at a cafe with coffee or a book, smart casual; (5) browsing a shop or market, casual chic; (6) cooking or tending plants at home, comfy loungewear. Movement and hand poses must look natural and anatomically plausible in every tile — no frozen or distorted mid-motion artifacts. Every outfit fully clothed with a relaxed, natural fit.

CRITICAL — FACE DETAIL AT DISTANCE: even in full-length tiles the face must stay sharp, clearly resolved, and recognizable — not soft or blurry. Render the face at higher effective detail than the surroundings if needed; background may be softly blurred, the face must stay in crisp focus.

LOCK (identical): facial identity and ALL skin characteristics; hair color/texture/shine/thickness/length; natural realistic body build and proportions. VARY only: activity/pose (per the list above), outfit per activity, setting/background, body/face orientation (balanced left/right), camera height/distance (within the framing distribution), lighting, hairstyle ARRANGEMENT.
...
def downgrade() -> None:
    # Data-content-only migration (no schema change). The prior 0023 prompt
    # text is superseded; downgrade is a no-op rather than restoring the
    # single-stage full-body lifestyle prompt. The column itself is dropped
    # by 0022's downgrade if needed.
    pass
